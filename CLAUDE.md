...
## Архитектура
- **Без сборки.** `index.html` грузит по порядку: `data.js` → `signals.js` → `chart.js` → `app.js` → `app-2.js` → `app-3.js` → `app-4.js` → `app-5.js` (классические `<script>`, общая глобальная область; функции + inline `onclick`). Затем — инлайновая регистрация SW.
