...
2. **Planning** — сформулировать задачу, согласовать подход, при объёмной фиче зафиксировать план до кода.
3. **Implementation** — реализация в нужной части (новая сессия/итерация на каждую фазу).
4. **Automated tests** — покрыть чистые функции в `tests/cases-app.js`, прогнать `bash tests/run.sh` + per-file JSC-синтаксис.
5. **Code review** — самопроверка диффа (агентно/вручную), сверка с доменными правилами и архитектурой.
6. **Manual testing** — проверка в реальном окружении (сайт/воркер), без моков.
7. **Documentation** — описание изменений (коммит/PR из диффа), обновление CLAUDE.md/памяти при необходимости.
Многофазные фичи: после этапа 6 возвращаемся к этапу 3 для следующей фазы. Коммит/пуш — только по явной просьбе (см. ниже).
