...
const DESK_IDEA_CFG={
  ...
  // Линия прибыли на графике «Акции» (plans/earnings-line.md): «нормальный» P/E берётся, только если в [peMin, peMax];
  // своя история P/E — медиана за ownYears лет (нужно ≥ ownMin), берётся, если разброс по годам (max ÷ min) ≤ dispX и она
  // (как и медиана сектора) отличается от текущего P/E не больше чем в histX раз (аудит 2026-09-11, 379 бумаг);
  // будущее — до futureBars пустых слотов (рабочих дней), в начальном окне видно viewFuture; линия вне
  // [min/scaleX, max×scaleX] цены окна не участвует в автомасштабе; прогноз, отличающийся от последнего отчёта больше
  // чем в jumpX раз, подписывается (разные базы: отчёт с разовыми статьями, консенсус — без них; аудит: 29 из 190).
  earn:{peMin:5,peMax:60,ownYears:5,ownMin:2,dispX:3,histX:2,jumpX:2,futureBars:520,viewFuture:20,scaleX:1.5}
};
