...
### Итоги E5b (2026-09-14, Sonnet 5 · high)

По §5 плана: новые чистые функции `pgEqVal` (значение `eq.` в кавычках — PostgREST требует их для `,.():`, ключи вкладок содержат скобки/эмодзи), `aiReportLastAtUrl`, `pfaEntry(a)`; async `aiReportLastAt(env,userId,kind,key)`/`aiReportInsert(env,userId,kind,key,at,data)` — тот же паттерн сервис-ключа, что `loadAlertState`/`saveAlertState`. `analyzeOnePortfolio`: гейт — `max(aiReportLastAt(...,'pfa',key) ?? 0, snap.data[key].pfAnalysisAt ?? 0)` (второе — переходный фолбэк на случай первого прогона после деплоя, пока в таблице ещё нет строк; поле само воркер больше не пишет); результат — `aiReportInsert(...,'pfa',key,entry.at,pfaEntry(a))` вместо `writeChecked` (в ledger `d.analysis`/`d.analysisHistory`/`d.pfAnalysisAt` больше не пишутся); Telegram — только после `true`, иначе `Анализ <key>: не удалось сохранить (ai_reports — выполнен ли supabase-ai-reports.sql?) — Telegram не отправлен` (§9 риск). Комментарии «пишется в data[key].analysis» (:51, :1974) поправлены на «ai_reports (kind 'pfa')». `WORKER_BUILD` → `2026-09-14e5b-ai-reports`, в `?action=version` добавлен тег `ai-reports`. Бюджет подзапросов не растёт (было `loadRow`+`writeChecked`≥2; стало `loadRow`+GET гейта+POST).

**Тесты.** worker-suite: группа `ai_reports (E5b)` (URL на тикере/на ключе со скобками/на ключе с эмодзи/на ключе с экранируемой кавычкой, `pfaEntry` отбрасывает лишние поля) — worker 239→244. `tests/run-worker-async.js`: мок `/rest/v1/ai_reports` (POST — upsert-игнор дублей по kind+key+at, `w.aiReportsFail` для сбоя вставки; GET — последняя строка по `kind`+`key`, разбор PostgREST-кавычек в моке зеркалит клиентский формат); переписаны ассерты групп «усечение» и «дедуп ошибок» с `w.snap.data[...].pfAnalysisAt/.analysis` на `w.aiReports`/`w.patches` (ledger больше не пишется); новая группа «гейт и запись в ai_reports (E5b)» — «Рано» по свежей строке таблицы, `force` игнорирует гейт, фолбэк на устаревший `pfAnalysisAt` пока таблица пуста, ошибка вставки → без Telegram и без ledger; async 46→50. `bash tests/run.sh` зелёный: app 1465 / worker 244 / async 50.

**За пользователем (§8 E5b):** деплой воркера вручную → Skill `verify-worker` (`?action=version` = `2026-09-14e5b-ai-reports`) → при наличии кредитов Anthropic — ручной `?action=pfanalyze&key=Portfolio (Anna)` или дождаться cron-слота: новая строка `pfa` (проверить `select * from ai_reports where kind='pfa' order by at desc limit 5`), в ledger `tabs_with_ai` больше не растёт, «📈 Анализ» обновляется без перезагрузки (realtime канал `ai_<uid>`), Telegram пришёл. Без кредитов — путь ошибки не изменился (ждёт пополнения, дедуп §"Дедуп ошибок анализа" работает как раньше — он не менялся).

**E5 закрыт** (E5a + E5b). Откат E5b — план §10 (`git revert` + деплой прежнего воркера: анализ снова пишется в ledger, новый клиент его переносит; данные текущих строк `ai_reports` не теряются). Дальше по блоку E — **E2 → E4** (`plans/ledger-model-e.md` §10 «Итоги E1»/§«Порядок»).
