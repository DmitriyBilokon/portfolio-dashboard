...
**За пользователем (по порядку).**
1. ~~Push~~ — запушено 2026-09-14 (6c87816, CLIENT_BUILD `68cdac02`). На **каждом** устройстве жёсткая перезагрузка (Cmd+Shift+R / сброс SW, PWA на телефоне тоже), в консоли `CLIENT_BUILD` = `?v=` нового `app.js`. Старый клиент по-прежнему теряет свою правку при равном rev и потом затирает чужие — пока он открыт, E4 защищает только новых.
2. ~~SQL~~ — пройдено 2026-09-15: строка владельца rev 7546, `cv` = `68cdac02`, `wid` непуст (последнюю запись в 08:00 UTC, вероятно, сделал воркер — он копирует `cv`/`wid` прочитанной строки; `wid` там бывает только после записи нового клиента); две другие строки не менялись с 04.09 и 16.06 — `NULL`, появятся при их входе. Запрос — пишет ли уже новый клиент (у его записей есть `wid`):
   ```sql
   select left(user_id::text,8) usr, data->>'rev' rev, data->>'cv' cv, data->>'wid' wid, updated_at
   from public.ledger_state order by updated_at desc;
   ```
3. ~~Живая проверка~~ — пройдена пользователем 2026-09-15 по всем пунктам (а–г): «всё работает». **E4 закрыт.** Что проверяли — **живая проверка §4.E4** (две вкладки браузера; «одновременно» надёжнее всего через офлайн): (а) вкладка B → DevTools → Network → Offline → правка в B (стоп позиции X); в A — правка **другой** позиции Y; B → Online → через секунду-две правка B в облаке, у A обе правки, тоста нет; (б) то же, но обе правят **одну** позицию X по-разному → в B тост «⚠ Одновременная правка: … · X — оставлена версия облака», в B значение A; (в) ноутбук «спит» (вкладка offline несколько минут) → правка → online → сохранена; (г) правка во время цикла AI-портфеля (▶) → и правка, и сделки AI на месте. В консоли слияние видно как `E4: одновременная правка…` (только при конфликтах). Если после Online правка B не ушла сразу — догон срабатывает не чаще раза в 30 с (`SYNC_RESUME_MS`): переключитесь на другую вкладку и обратно или сделайте в B ещё одну правку (её push получит отказ триггера и сольётся).
...
### Воркер: та же дыра равного rev (вне блока E4, `telegram-notify.js`)

**Сделано и закрыто 2026-09-15 (Sonnet 5).** `writeRow` пишет свой `wid` в данные (`workerWid()`, тот же принцип, что `syncWid` клиента); `writeCommitted(rows, expectedRev, expectedWid)` — коммит ⇔ rev И wid вернувшейся строки наши (без `expectedWid` — старое поведение, только по rev, использовалось в старых кейсах). Без этого отказ триггера при равном rev другого писателя (клиент + воркер, или два запуска воркера подряд) был неотличим от коммита — воркер мог молча потерять свою запись (сделку AI-портфеля, лимиты bookcheck, таргеты). `WORKER_BUILD` → `2026-09-15e4-wid-commit` (тег `wid-commit` в `?action=version`). Тесты: `writeCommitted` +3 в worker-suite (worker 264→267: rev совпал/wid наш → true, равный rev но чужой wid → false, равный rev без wid у ответа → false); новая группа в `tests/run-worker-async.js` — `writeRow` со своим фейковым fetch (обычный коммит, старый rev, **равный rev с чужим wid → false** — воспроизводит саму гонку, HTTP-ошибка) (worker-async 50→54). Приём: откат проверки wid во временной копии (`return true` без сравнения) — новый кейс «равный rev, но чужой wid» падает (проверено). `bash tests/run.sh` — app 1623, worker 267, worker-async 54, sync-sim 223. Запушено (28476bd), воркер задеплоен и проверен пользователем 2026-09-15: `?action=version` показывает `2026-09-15e4-wid-commit` и тег `wid-commit`; SQL после записи воркера — `wid` в строке владельца меняется на новое значение на каждой записи. **Закрыто.**
**Откат.** `git revert` + деплой воркера (`WORKER_BUILD` предыдущий — `2026-09-14e2-named-cols`). Данные не менялись.
