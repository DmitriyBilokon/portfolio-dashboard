...
См. `plans/ai-reports-e5.md` §13 «Итоги E5a» (SQL, клиент, отклонения, тесты, стенд, SQL-сверка и откат). Кратко: `supabase-ai-reports.sql`; слой `AI_REP` в app.js; перенос из ledger — только после подтверждённой вставки; `stockAiLog` — в снапшоте, только пока не перенесён; стенд `--ai-out` (I5); app 1465 тестов. Дальше — **E5b** (Sonnet 5 · high).
