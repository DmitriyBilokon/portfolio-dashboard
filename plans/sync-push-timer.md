...
Статус: **Implementation завершён** (2026-09-11, Fable · high): §3 реализован в `app.js` как в плане, §4 — группа `sync queue` в `tests/cases-app.js` (27 кейсов, 1313/1313 зелёных), §6 — CLAUDE.md, s7b-map §10, audit-followup E. Ручная проверка §5 пройдена пользователем 2026-09-11 после деплоя сайта («вроде всё ок»). Деплой воркера не нужен, SQL не меняется.
