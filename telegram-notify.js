...
const WORKER_BUILD = '2026-06-24betyg-ai';   // ?action=version — проверить, что задеплоено
...
- Опирайся на технику (положение относительно SMA/поддержки/сопротивления, перегрев, падающий нож), оценку (потенциал к таргету) и риск (концентрация, тип бумаги). У позиций может быть betyg (фундамент-рейтинг score100 0–100 + буква A–F) — используй как срез качества бизнеса: высокий оправдывает удержание/добор, низкий — кандидат на ротацию в более качественную идею. Для каждой рекомендации в details укажи уровень входа/выхода и краткую причину; amountSEK — ориентировочная сумма в kr или null.
...
async function buildPortfolioSnapshot(env, key, snap){
  ...
  for(const r of d.rows){
    ...
    positions.push({
      ...
      upsidePct: uRow[10],
      betyg: rowBetygLite(num(r, ix.roe), num(r, ix.revg)),   // 🏅 фундамент-рейтинг бумаги (буква A–F + 0–100)
    });
  }
