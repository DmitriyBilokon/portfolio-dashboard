...
const WORKER_BUILD = '2026-06-16learn';   // ?action=version — проверить, что задеплоено
...
async function anthropicRun(env, body){
  ...
  for(let round = 0; round < 6; round++){
    ...
    if(!summarize && j.stop_reason === 'pause_turn'){
      ...
      // Лимит на ресёрч: израсходовали ~5 минут → просим свести найденное БЕЗ новых поисков.
      if(Date.now() - started > AI_RESEARCH_MS){
        messages = messages.concat([{ role: 'user', content: 'Лимит веб-поиска (~5 минут) исчерпан. БОЛЬШЕ НЕ ИЩИ. Сведи уже найденную информацию и данные снапшота в финальный ответ строго в требуемом формате — кратко, по делу, с конкретикой и уровнями.' }]);
        summarize = true;
      }
      continue;
    }
...
ЖИВЫЕ НОВОСТИ И МАКРО. ОБЯЗАТЕЛЬНО используй web_search, чтобы собрать самые свежие данные: новости по ключевым позициям портфеля и кандидатам на новые покупки (отчёты, гайденс, сделки M&A, рейтинги и таргеты аналитиков, регуляторика) и глобальную макрокартину (ставки ФРС/ЕЦБ/Riksbank, инфляция, геополитика, цены на сырьё и валюты, настроение по секторам и ведущим индексам). Учитывай найденное во всех разделах и рекомендациях; кратко ссылайся на самое важное. БЮДЖЕТ РЕСЁРЧА ОГРАНИЧЕН (~5 минут): делай НЕСКОЛЬКО точечных запросов по самому важному (ключевые позиции, кандидаты, главный макро-драйвер), НЕ дублируй и не уходи в бесконечный поиск — затем ОБЯЗАТЕЛЬНО сведи найденное в выводы. Лучше меньше запросов и чёткое резюме, чем много поисков.
...
ОБЯЗАТЕЛЬНО используй web_search для свежих новостей и событий по компании (отчёты, гайденс, сделки, регуляторика, отраслевой фон) — на дату анализа. Кратко сошлись на найденное в разделе новостей. Бюджет поиска ограничен (~5 мин): 2–4 точечных запроса, без дублей, затем сведи найденное.
...
Бюджет поиска ограничен (~5 мин): 2–4 точечных запроса, без дублей, затем сведи найденное в вывод.
