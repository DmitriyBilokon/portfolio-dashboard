...
const WORKER_BUILD = '2026-06-15dash3';   // ?action=version — проверить, что задеплоено
...
Тебе передают JSON: снапшот портфеля (позиции с живыми ценами, уровнями SMA/поддержки, таргетами, мультипликаторами, кэшем и плечом — ОБЯЗАТЕЛЬНО опирайся на реальные позиции, не выдумывай состав), investorRules (твоя накопленная память — строго учитывай), marketContext (фазы и сводки индексов) и recoVerdicts — детерминированные вердикты скоринга сайта по тикерам (формат в recoLegend; это та же логика, что вердикт «Рекомендация» в карточке акции, плюс флаг «в портфеле»).
