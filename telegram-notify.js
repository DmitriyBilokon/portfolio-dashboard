...
🔄 РАЗВОРОТ НА ДНЕ — РАННИЙ ВХОД МАЛЫМ РАЗМЕРОМ (исключение из правила падающего ножа). Если бумага у многолетнего/циклического ДНА, НО фундаментал уже разворачивается В ПЛЮС — улучшается БАЛАНС (долг снижается, кэш/ликвидность растут, D/E падает), маржа/FCF разворачиваются вверх, выручка стабилизируется или снова растёт, betyg/качество улучшается (подтверди по revSeries/fundamentals/свежим новостям) — это НЕ падающий нож, а кандидат на РАННИЙ вход. Тогда рекомендуй СТАРТОВУЮ позицию МАЛЫМ траншем (не полный размер) с планом добора при ПОДТВЕРЖДЕНИИ разворота (возврат над SMA50, серия higher-low, позитивный отчёт). Чётко отличай от value-trap: нужен КОНКРЕТНЫЙ фактический сдвиг фундаментала/баланса, а не только «дёшево» и большой апсайд к устаревшему таргету. Пока разворот тренда не подтверждён — размер позиции мал.
...
КАЧЕСТВЕННЫЙ РОСТ НА ОТКАТЕ И ДОЛГОЕ УДЕРЖАНИЕ КАЧЕСТВА (выбор новых позиций, куда идёт кэш, и горизонт). (1) Новыми деньгами предпочитай качественные РАСТУЩИЕ компании (высокий ROE/маржа, умеренный долг, +FCF; выручка растёт двузначно; есть структурный драйвер) в зоне ЗДОРОВОЙ ПРОСАДКИ: цена у SMA50 при целом восходящем тренде, с запасом хода к таргету. НЕ открывай и не докупай новыми деньгами имена НА ПИКЕ (растянуты примерно >20–30% над SMA200, БЛИЗКО К СОПРОТИВЛЕНИЮ / у верхней границы диапазона, эйфоричная оценка, апсайд к таргету ~0, recoVerdict=sell/wait по перегреву) — асимметрия против тебя; и НЕ закрывай недовесы чистым дефенс/дивидендным балластом или падающими ножами без роста (сломанный тренд / отрицательный рост выручки) — выбирай рост-имена на откате. ЖЁСТКОЕ ПРАВИЛО: закрытие недовеса сектора/гео САМО ПО СЕБЕ НЕ оправдывает покупку — если качественное имя нужно для диверсификации, но оно у сопротивления / растянуто / апсайд мал, помести его в watchlist с условием входа (откат к SMA50/поддержке), а НЕ в «Купить/Докупить» сейчас. «Ради диверсификации» — недопустимое единственное обоснование докупки у сопротивления. (2) КАЧЕСТВО МОЖНО ДЕРЖАТЬ ДОЛГО: качественную бумагу (высокий betyg, крепкий ROE/маржа/FCF, целый тезис) можно оставлять в портфеле НА ДОЛГИЙ срок, через циклы и просадки — длинный горизонт удержания для качества норма, а не риск; не выгоняй качество ради оборота, «ровных долей» или краткосрочной техники. Продавай качество только при объективной деградации (слом тезиса/ухудшение фундаментала, цена устойчиво и сильно выше справедливой оценки, опасная концентрация); частичная фиксация на силе допустима, но ЯДРО качественной позиции держим долгосрочно (бонус: низкий оборот бережёт издержки и налог genomsnittsmetoden). Это НЕ отменяет правил выше про удержание победителей — принцип (1) лишь направляет СВЕЖИЙ капитал, принцип (2) задаёт долгий горизонт для качества.
...
- КАЧЕСТВЕННЫЙ РОСТ НА ОТКАТЕ (новые покупки и куда идёт кэш). Новыми деньгами предпочитай качественные РАСТУЩИЕ имена (высокий ROE/маржа, +FCF, двузначный рост выручки, структурный драйвер) в зоне здоровой просадки — у SMA50 при целом аптренде, с запасом к таргету. НЕ покупай новыми деньгами имена на пике (растянуты примерно >20–30% над SMA200, близко к сопротивлению / у верхней границы диапазона, оценка эйфоричная, апсайд к таргету ~0, recoVerdict=sell/wait по перегреву) и не закрывай недовесы дефенс/дивидендным балластом или падающими ножами без роста — бери рост на откате. Закрытие недовеса сектора само по себе НЕ оправдывает покупку у сопротивления: качественное имя у сопротивления НЕ покупай ради ребаланса — жди отката к SMA50/поддержке.
- КАЧЕСТВО ДЕРЖИМ ДОЛГО. Качественную бумагу (высокий betyg, целый тезис) можно держать в портфеле долго, через циклы и просадки — не выгоняй её ради оборота/ребаланса/краткосрочной техники. Продавай качество только при сломе тезиса/ухудшении фундаментала, устойчивой сильной переоценке или опасной концентрации; частичная фиксация на силе ок, ядро держим долгосрочно.
...
- 🔄 РАЗВОРОТ НА ДНЕ — стартовая позиция малым размером (исключение из «падающего ножа»). Если бумага у циклического/многолетнего дна, НО фундаментал/баланс уже разворачиваются в плюс (долг ↓, кэш ↑, D/E ↓, маржа/FCF ↑, выручка стабилизируется/растёт) — это не нож, а кандидат на РАННИЙ вход СТАРТОВЫМ малым траншем с добором при подтверждении разворота (возврат над SMA50). Нужен КОНКРЕТНЫЙ сдвиг фундаментала, а не только дешевизна/устаревший апсайд; размер мал, пока тренд не подтверждён.
...
- 🔄 РАЗВОРОТ НА ДНЕ — «Купить» малым размером (исключение из «падающего ножа»). Если бумага у дна, НО баланс/фундаментал разворачиваются в плюс (долг ↓, кэш ↑, D/E ↓, маржа/FCF ↑, выручка стабилизируется/растёт по revSeries) — рекомендуй стартовый МАЛЫЙ транш с добором при подтверждении разворота (возврат над SMA50), а не «Избегать». Нужен конкретный сдвиг фундаментала, а не только дешевизна/устаревший апсайд (иначе это value-trap).
- КАЧЕСТВЕННЫЙ РОСТ НА ОТКАТЕ (новые идеи и куда идёт кэш). Рекомендуй к покупке/докупке прежде всего качественные РАСТУЩИЕ компании (высокий ROE/маржа, +FCF, двузначный рост, структурный драйвер) в здоровой просадке — у SMA50 при целом тренде, с запасом к таргету. НЕ предлагай к покупке имена на пике (растянуты примерно >20–30% над SMA200, близко к сопротивлению / у верхней границы диапазона, оценка эйфоричная, апсайд ~0, recoVerdict=sell/wait по перегреву) и не закрывай недовесы чистым дефенс/дивидендным балластом или падающими ножами без роста — выбирай рост на откате. ЖЁСТКОЕ ПРАВИЛО: закрытие недовеса сектора НЕ оправдывает «Купить/Докупить» у сопротивления — такую идею ставь «Держать»/«Наблюдать» с условием входа (откат к SMA50/поддержке), а не «Купить» сейчас.
- КАЧЕСТВО ДЕРЖИМ ДОЛГО. Качественную бумагу (высокий betygLite/качество бизнеса, целый тезис) можно держать долго через циклы — по умолчанию «Держать», а не «Сократить/Продать» ради оборота или краткосрочной техники. Рекомендуй выход из качества только при сломе тезиса/ухудшении фундаментала, устойчивой сильной переоценке или опасной концентрации.
...
🔄 РАЗВОРОТ НА ДНЕ — ДОБАВЛЯТЬ малым размером (исключение из правила падающего ножа). Если бумага у многолетнего/циклического ДНА, НО фундаментал уже разворачивается в плюс — улучшается БАЛАНС (долг снижается, кэш/ликвидность растут, D/E падает), маржа/FCF разворачиваются вверх, выручка стабилизируется/снова растёт, betyg улучшается (проверь по fundamentals/revSeries/web_search) — это НЕ падающий нож. Тогда на горизонтах «Момент»/лонг оправдано ДОБАВЛЯТЬ СТАРТОВОЙ малой позицией (не полный размер) с планом добора при подтверждении разворота (возврат над SMA50, серия higher-low, позитивный отчёт). Отличай от value-trap: нужен КОНКРЕТНЫЙ фактический сдвиг фундаментала/баланса, а не только дешевизна и большой апсайд к устаревшему таргету. Размер мал, пока тренд не подтверждён.
...
- 🔄 РАЗВОРОТ НА ДНЕ (исключение из «падающего ножа»): бумага у многолетнего/циклического дна + улучшающийся БАЛАНС/фундаментал (долг ↓, кэш/ликвидность ↑, D/E ↓, маржа/FCF разворот вверх, выручка стабилизируется/растёт, betyg ↑) — это НЕ «avoid», а кандидат на «buy» МАЛЫМ размером (стартовая позиция) с добором при подтверждении разворота (возврат над SMA50, higher-low, позитивный отчёт). Нужен КОНКРЕТНЫЙ фактический сдвиг фундаментала, а не только дешевизна и большой апсайд к устаревшему таргету (иначе value-trap). В note отрази малый размер до подтверждения.
...
КАЧЕСТВЕННЫЙ РОСТ НА ОТКАТЕ И ДОЛГОЕ УДЕРЖАНИЕ КАЧЕСТВА (для picks и карточек возможностей/рисков). (1) В picks и «возможностях» новый кэш направляй в качественные РАСТУЩИЕ имена (высокий ROE/маржа, +FCF, двузначный рост, структурный драйвер) в здоровой просадке — у SMA50 при целом тренде, с запасом к таргету; НЕ давай вердикт «Купить/Докупить» именам на пике (растянуты примерно >20–30% над SMA200, близко к сопротивлению / у верхней границы диапазона, оценка эйфоричная, апсайд ~0, recoVerdict=sell/wait по перегреву) и не закрывай недовесы дефенс/дивидендным балластом или падающими ножами без роста. Закрытие недовеса сектора НЕ оправдывает «Купить/Докупить» у сопротивления — ставь «Наблюдать» с условием входа на откате (к SMA50/поддержке), а не «Купить» сейчас. (2) Качественную бумагу (высокий betyg, целый тезис) можно держать долго через циклы — по умолчанию «Держать», не помечай её «Сократить/Продать» ради оборота или краткосрочной техники; выход из качества — только при сломе тезиса/ухудшении фундаментала, устойчивой сильной переоценке или опасной концентрации. (3) 🔄 РАЗВОРОТ НА ДНЕ: бумага у дна, НО баланс/фундаментал разворачиваются в плюс (долг ↓, кэш ↑, маржа/FCF ↑, выручка стабилизируется/растёт) — не «падающий нож»: в picks/возможностях дай вход СТАРТОВЫМ малым размером с добором при подтверждении (возврат над SMA50). Нужен конкретный сдвиг фундаментала, а не только дешевизна.
