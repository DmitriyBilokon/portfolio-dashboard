...
// ── Грузим реальный signals.js + app.js…app-5.js (в порядке как в index.html; без авто-boot) + фикстуры и кейсы в ОДНОМ eval ──
var appSrc = ['signals.js','app.js','app-2.js','app-3.js','app-4.js','app-5.js'].map(rd).join('\n').replace(/\nboot\(\);\s*$/, '\n');
